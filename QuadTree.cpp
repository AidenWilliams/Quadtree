...
bool Quad::arePointersThere(Quad quad) {
    return quad.topRightTree != nullptr &&
           quad.botLeftTree != nullptr &&
           quad.topRightTree != nullptr &&
           quad.botRightTree != nullptr;
}

void Quad::compress(int loss) {
    if(n == nullptr){
        if(arePointersThere(*this)){
            topLeftTree->compress(loss);
            botLeftTree->compress(loss);
            topRightTree->compress(loss);
            botRightTree->compress(loss);
        }else{
           // n = ;
        }
    }
